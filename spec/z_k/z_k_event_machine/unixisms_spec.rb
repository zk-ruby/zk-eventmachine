require 'spec_helper'

module ZK::ZKEventMachine
  describe 'Unixisms' do
    include EventedSpec::SpecHelper
    default_timeout 2.0

    before do
      @zk = ::ZK.new
      @base_path = '/zk-em-testing'
    end

    after do
      @zk.rm_rf(@base_path)
      @zk.close!
    end

    def close_and_done!
      if @zkem
        @zkem.close! { done }
      else
        done
      end
    end

    describe 'chrooted connection' do
      before do
        @zkem = ZK::ZKEventMachine::Client.new("localhost:2181#{@base_path}")
      end

      describe 'with a non-existent root' do
        before do
          @zk.rm_rf(@base_path)
          @bogus_path = '/this/will/not/work'
        end

        describe 'mkdir_p' do
          it %[should fail node style] do
            em do
              @zkem.connect do
                @zk.exists?(@base_path).should_not be_true
                @zk.exists?(File.join(@base_path, @bogus_path)).should_not be_true

                @zkem.mkdir_p(@bogus_path) do |exc,_|
                  exc.should_not be_nil
                  close_and_done!
                end
              end
            end
          end # node style

          it %[should fail callback style] do
            em do
              @zkem.connect do
                @zk.exists?(@base_path).should_not be_true
                @zk.exists?(File.join(@base_path, @bogus_path)).should_not be_true

                @zkem.mkdir_p(@bogus_path).callback do |*|
                  raise "no no no! you should fail!"
                end.errback do |exc|
                  exc.should_not be_nil
                  close_and_done!
                end
              end
            end
          end # node style


        end # mkdir_p
      end # with a non-existent root
    end # chrooted connection

    describe 'normal connection' do
      before do
        @zk.rm_rf(@base_path)
        @zk.mkdir_p(@base_path)
        @zkem = ZK::ZKEventMachine::Client.new('localhost:2181')
      end

      describe 'mkdir_p' do
        before do
          @bogus_paths = [
            [@base_path, 'bogus', 'path', 'to', 'qwer'].join('/'),
            [@base_path, 'bogus', 'path', 'to', 'somethingelse'].join('/')
          ]
        end

        it %[should create the path recursively] do
          @zk.exists?(@bogus_paths.first).should be_false

          em do
            @zkem.connect do
              @zkem.mkdir_p(@bogus_paths.first).callback do |p|
                p.first.should == @bogus_paths.first
                close_and_done!
              end.errback do |e|
                raise e
              end
            end
          end
        end

        it %[should not error on a path that already exists] do
          @zk.mkdir_p(@bogus_paths.first)

          em do
            @zkem.connect do
              @zkem.mkdir_p(@bogus_paths.first) do |exc,p|
                exc.should be_nil
                p.first.should == @bogus_paths.first
                close_and_done!
              end
            end
          end
        end

        it %[should take an array of paths] do
          @bogus_paths.each do |p|
            @zk.exists?(p).should be_false
          end

          em do
            @zkem.connect do
              @zkem.mkdir_p(@bogus_paths) do |exc,paths|
                exc.should be_nil
                paths.should be_kind_of(Array)
                @bogus_paths.each { |p| paths.should include(p) }
                close_and_done!
              end
            end
          end
        end
      end # mkdir_p

      describe 'rm_rf' do
        em_before do
          @relpaths = ['disco/foo', 'prune/bar', 'fig/bar/one', 'apple/bar/two', 'orange/quux/c/d/e']

          @roots = @relpaths.map { |p| File.join(@base_path, p.split('/').first) }.uniq
          @paths = @relpaths.map { |n| File.join(@base_path, n) }

          @paths.each { |n| @zk.mkdir_p(n) }
        end

        it %[should remove the paths recursively] do
          em do
            @zkem.connect do
              @zkem.rm_rf(@roots).callback do
                @roots.each { |p| @zk.exists?(p).should be_false }
                close_and_done!
              end.errback do |exc|
                raise exc
              end
            end
          end
        end # it

        it %[should use the nodejs style if a block is given] do
          em do
            @zkem.connect do
              @zkem.rm_rf(@roots) do |exc|
                if exc.nil?
                  @roots.each { |p| @zk.exists?(p).should be_false }
                  close_and_done!
                else
                  raise exc
                end
              end
            end
          end
        end
      end # rm_rf
    end # normal connection
  end # Unixisms
end



